        ...
        public virtual void SetNextPosition(Level environment, ContentManager contentManager)
        {
            ...
            var tileLowerLeft = environment.GetTile(TileCheckArray.LowerLeft.X = (int) Math.Ceiling(Position.X / 16 - 1),
                TileCheckArray.LowerLeft.Y =
                    (int) Math.Floor(Position.Y / 16 + Math.Ceiling(1.0)));
            var tileUpperLeft = environment.GetTile(TileCheckArray.UpperLeft.X = (int) Math.Ceiling(Position.X / 16 - 1),
                TileCheckArray.UpperLeft.Y =
                    (int) Math.Floor(Position.Y / 16 + Math.Ceiling(0.0)));
            var tileLowerRight = environment.GetTile(TileCheckArray.LowerRight.X = (int) Math.Floor(Position.X / 16 + 1),
            ...
            TileCheckArray.LowerLeft.TileId = tileLowerLeft;
            TileCheckArray.LowerRight.TileId = tileLowerRight;
            TileCheckArray.UpperLeft.TileId = tileUpperLeft;
            TileCheckArray.UpperRight.TileId = tileUpperRight;

            var velocity = Velocity;
            // Logging.Info($"{environment._entities.IndexOf(this)} Checking tile at {Position.X / 16}, {Position.Y / 16 + GetTexture2D().Height}");
            switch (CollisionType)
            {
                case CollisionType.TwoBlockFull:
                case CollisionType.TwoBlockPartial:
                    ...
                    if (CollisionType == CollisionType.TwoBlockFull)
                    {
                ...
                case CollisionType.OneBlockFull:
                case CollisionType.OneBlockPartial:
                    break;
            }
